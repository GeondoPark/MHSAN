...
from utils import AverageMeter, save_checkpoint, collate_fn_padded, log_epoch, adjust_learning_rate
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Flickr_DATASET_TRAIN')
    ...
    parser.add_argument("-lr_decay_list", "--lr_decay_list", type=int, default=[3, 7, 52],
                        help="List of epoch where the learning rate is decreased"
                        "(multiplied by first arg of lrd)", nargs='+')
    parser.add_argument("-lr_decay", "--lr_decay", nargs='+', default=[0.2, 0.5, 0.1],
                        help="List of epoch where the learning rate is decreased"
    ...
    shutil.copyfile('model.py', path_weights+'/{}_model.py'.format(args.model_name))
    shutil.copyfile('train.py', path_weights+'/{}_train.py'.format(args.model_name))    
    ...
    flickr_data_train = FlickrDataset(split="train", transform=prepro)
    flickr_data_val = FlickrDataset(split="val", transform=prepro_val)

    train_loader = DataLoader(flickr_data_train, batch_size=args.batch_size, shuffle=True,
                              num_workers=4, collate_fn=collate_fn_padded, pin_memory=True)
    val_loader = DataLoader(flickr_data_val, batch_size=args.batch_size, shuffle=False,
                            num_workers=4, collate_fn=collate_fn_padded, pin_memory=True)
    ...
    for epoch in range(0, args.max_epoch):
        ...
        if epoch == 3:
            print("\nSarting finetuning Image Encoder\n")
            finetune = True

            for param in join_emb.cap_emb.module.parameters():
            	param.requires_grad = True

            for a, b in join_emb.named_parameters():
                if b.requires_grad == True:
                    print(a, b.requires_grad)

            optimizer.add_param_group({'params': filter(lambda p: p.requires_grad, join_emb.cap_emb.module.parameters()), 'lr': optimizer.param_groups[0]['lr']})
        ...
        if epoch == 40:
            ...
            optimizer.add_param_group({'params': filter(lambda p: p.requires_grad, join_emb.img_emb.module.parameters()), 'lr': optimizer.param_groups[0]['lr']})
